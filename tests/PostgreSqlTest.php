...
// PostgreSQL parity for the ORM. The same db fixture and probes that DbTest runs on SQLite run here
// against a real PostgreSQL (PHLO_TEST_DB=pgsql, creds via PHLO__postgresql__* env). Skipped when no
// PostgreSQL is configured, so the default suite stays dependency-free; CI runs it against a Postgres
// service. Covers the behaviours that differ on Postgres: CRUD + the ON CONFLICT DO NOTHING
// insert-ignore path, a custom string primary key (whose insert touches no sequence, so lastId() must
// not let a failed lastval() poison the transaction - the SAVEPOINT guard in PostgreSQL.phlo),
// relation loading, and the audit feature's transaction + savepoint rollback with a quoted "user"
// column (a reserved word in Postgres).
final class PostgreSqlTest extends TestCase {
	...
	public static function setUpBeforeClass():void {
		if (getenv('PHLO_TEST_DB') !== 'pgsql') self::markTestSkipped('PostgreSQL parity test: set PHLO_TEST_DB=pgsql and PHLO__postgresql__* to run (CI does; the default suite is SQLite-only)');
		[$code, $out, $err] = self::cli('build::run');
		self::assertSame(0, $code, "build::run failed:\n$out$err");
	}

	public function testOrmOnPgsql():void {
		$r = self::fetch('note::runTests');
		$this->assertTrue($r['create'] ?? false, 'create');
		$this->assertSame([1, 2], $r['ids'] ?? null, 'SERIAL ids');
		...
		$this->assertTrue($r['ignoreNoNewRow'] ?? false, 'INSERT ... ON CONFLICT DO NOTHING adds no row on conflict (the Postgres insert-ignore differs from MySQL INSERT IGNORE and SQLite INSERT OR IGNORE)');
		$this->assertTrue($r['ignoreNoOverwrite'] ?? false, 'ON CONFLICT DO NOTHING does not overwrite');
	...
	public function testCustomStringPrimaryKeyOnPgsql():void {
		$r = self::fetch('code::runTests');
		$this->assertTrue($r['create'] ?? false, 'create reloads the row by its custom VARCHAR primary key - its insert touches no sequence, so lastId() must not poison the transaction');
	...
	public function testRelationLoadingOnPgsql():void {
	...
	public function testAuditTransactionsAndSavepointsOnPgsql():void {
		$logged = self::fetch('audited::runTests');
		$this->assertSame(1, $logged['logged'] ?? null, 'an audited change is logged (the audit insert quotes the reserved "user" column)');
		...
		$this->assertSame(1, $save['logged'] ?? null, 'objSave() of a new audited record (custom PK, inside the audit transaction) logs a create');
	}
}
