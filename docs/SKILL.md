...
**Raw streams.** The `stream` resource carries data that does not fit the JSON command channel. Server-side, `stream($data, $type, $name)` opens a streaming response under any content type (default `application/octet-stream`) and emits string or iterable chunks verbatim; call it repeatedly to append, pass `$name` for a download filename. Frontend, `app.stream(path, onData = null, async = false, type = null, data = null)` consumes such a response with fetch and dispatches on the Content-Type: `ndjson` and `sse` parse per line/frame (SSE line endings are normalized, so CRLF streams frame correctly), `text` streams decoded text, `json` and `blob` resolve once, anything else streams raw `Uint8Array` chunks and resolves to a `Blob`. `onData` receives each chunk, `type` overrides the detection, `data` posts a body. The CSRF token is always sent; `X-Requested-With: phlo` only when `async` is true, which is required to reach a `route async` target - without it the request counts as a plain page request.
