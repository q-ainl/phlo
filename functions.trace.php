<?php
function error(string $msg, int $code = 500):never {
	trace('error', compact('msg', 'code'));
	throw new PhloException($msg, $code);
}
function esc($string):string {
	trace('esc', compact('string'));
	return htmlspecialchars((string)$string, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');
}
function arr(...$array):array {
	trace('arr', compact('array'));
	return $array;
}
function obj(...$data):obj {
	trace('obj', compact('data'));
	return new obj(...$data);
}
function first(...$args):mixed {
	trace('first', compact('args'));
	return current($args);
}
function last(...$args):mixed {
	trace('last', compact('args'));
	return end($args);
}

function loop(iterable $data, Closure|array $cb, ?string $implode = null):mixed {
	trace('loop', compact('data', 'cb', 'implode'));
	$return = [];
	$isArray = is_array($cb);
...
function files(string|array $paths, string $ext = '*.*'):array {
	trace('files', compact('paths', 'ext'));
	return array_merge(...loop((array)$paths, fn($path) => glob("$path$ext") ?: []));
}
function dirs(string $path):array {
	trace('dirs', compact('path'));
...
function json_read(string $file, ?bool $assoc = null):mixed {
	...
	return json_decode((string)file_get_contents($file), $assoc) ?? error('Error reading '.esc($file));
}
function regex(string $pattern, string $subject, int $flags = 0, int $offset = 0):array {
	trace('regex', compact('pattern', 'subject', 'flags', 'offset'));
	return preg_match($pattern, $subject, $match, $flags, $offset) ? $match : [];
}
function regex_all(string $pattern, string $subject, int $flags = 0, int $offset = 0):array {
	trace('regex_all', compact('pattern', 'subject', 'flags', 'offset'));
	return preg_match_all($pattern, $subject, $matches, $flags, $offset) ? $matches : [];
}
function is_absolute_url(string $url):bool {
	trace('is_absolute_url', compact('url'));
	return str_starts_with($url, 'http://') || str_starts_with($url, 'https://');
}
function shortpath(?string $file):string {
	trace('shortpath', compact('file'));
...
function route(?string $method = null, string $path = void, ?bool $async = null, ?string $data = null, ?callable $cb = null):bool|obj {
	trace('route', compact('method', 'path', 'async', 'data', 'cb'));
...
function indent(string $string, int $depth = 1):string {
	trace('indent', compact('string', 'depth'));
	return ($tab = str_repeat(tab, $depth)).rtrim(strtr($string, [lf => lf.$tab]), tab);
}
function indentView(string $string, int $depth = 1):string {
	trace('indentView', compact('string', 'depth'));
	return last($tab = str_repeat(tab, $depth), rtrim(preg_replace('/\n(\t*)</', "\n$1$tab<", $string), tab));
}

function DOM(string $body = void, string $head = void, string $lang = 'en', string $bodyAttrs = void, string $htmlAttrs = void):string {
	trace('DOM', compact('body', 'head', 'lang', 'bodyAttrs', 'htmlAttrs'));
	return "<!DOCTYPE html>\n<html lang=\"$lang\"$htmlAttrs>\n<head>\n$head</head>\n<body$bodyAttrs>\n$body\n</body>\n</html>";
}

function title(?string $title = null, string $implode = ' - '):string {
	trace('title', compact('title', 'implode'));
	$res = phlo('res');
...
function phlo_auth(string $name, ?string $realm = null):bool {
	trace('phlo_auth', compact('name', 'realm'));
