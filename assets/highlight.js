function highlight_Phlo(source){
  ...
  const PHP_FUNCS = new Set([
    'array_merge', 'count', 'in_array', 'strlen', 'strpos', 'str_replace', 'json_encode', 'preg_match',
  ...
  const highlightTag = raw => {
    ...
    while (i < n){
      ...
      if (i > attrStart){
        out += sp('hl-attr-name', esc(raw.slice(attrStart, i)))
        const ws = i
        while (i < n && /\s/.test(raw[i])) i++
        out += raw.slice(ws, i)
        if (i < n && raw[i] === '='){
          ...
          if (i < n && (raw[i] === '"' || raw[i] === "'")){
            ...
            while (i < n && raw[i] !== q) i++
            if (i < n) i++
            out += sp(q === '"' ? 'hl-string-dq' : 'hl-string-sq', esc(raw.slice(valueStart, i)))
          }
          else {
            const valueStart = i
            while (i < n && !/[\s>]/.test(raw[i])) i++
            out += sp('hl-attr-value', esc(raw.slice(valueStart, i)))
          }
        }
        continue
      }
  ...
  for (let li = 0; li < lines.length; li++){
    ...
    const cm = line.slice(lead)
    let painted = false
    let nextMember = false
    let res = ''
    let last = 0
    if (cm.startsWith('#') || cm.startsWith('//')){
      out += esc(line.slice(0, lead)) + sp('hl-cmt', esc(line.slice(lead))) + (li < lines.length - 1 ? '\n' : '')
      continue
    }
    rx.lastIndex = 0
    let it
    while ((it = rx.exec(line))){
      ...
      last = ofs + tok.length
      if (tok[0] === '<'){
        res += highlightTag(tok)
        continue
      }
      if (ofs === lead && !painted && /^[A-Za-z_]\w*$/.test(tok) && NODES.has(tok)){
        res += sp('hl-node', esc(tok))
        painted = true
        continue
      }
      if (tok === '->'){
        res += sp('hl-objop', esc(tok))
        nextMember = true
        continue
      }
      if (tok === '::'){
        res += sp('hl-classop', esc(tok))
        nextMember = true
        continue
      }
      if (tok === '{{' || tok === '}}' || tok === '{(' || tok === ')}'){
        res += sp('hl-operator', esc(tok))
        continue
      }
      if (tok[0] === '"' || tok[0] === "'"){
        res += sp(tok[0] === '"' ? 'hl-string-dq' : 'hl-string-sq', esc(tok))
        continue
      }
      if (tok[0] === '%' && /^%[A-Za-z_]\w*$/.test(tok)){
        res += sp('hl-obj', esc(tok))
        continue
      }
      if (tok[0] === '$' && /^\$[A-Za-z_]\w*$/.test(tok)){
        res += sp('hl-var', esc(tok))
        continue
      }
      if (/^\d/.test(tok) || /^\.\d/.test(tok)){
        res += sp('hl-number', esc(tok))
        continue
      }
      if (['(', ')', '[', ']', '{', '}', ';', ',', '.', '=', '=>', '<=', '>=', '===', '==', '!=', '&&', '||', ':', '-', '+', '*', '/', '|', '&', '<', '>', '?'].includes(tok)){
        res += sp('hl-operator', esc(tok))
      ...
      if (/^[.%A-Za-z0-9_-]+$/.test(tok) && last < line.length && line.slice(last, last + 2) === ': ' && (ofs === 0 || /\s/.test(line[ofs - 1] || ''))){
        res += sp('hl-key', esc(tok))
        continue
      }
      if (/^[A-Za-z_]\w*$/.test(tok)){
        if (nextMember){
          res += sp('hl-member', esc(tok))
          nextMember = false
          continue
        }
        const lower = tok.toLowerCase()
        if (CONSTS.has(lower)){
          res += sp('hl-const', esc(tok))
          continue
        }
        if (/^\s*\(/u.test(line.slice(last))){
          if (PHP_FUNCS.has(tok)){
            res += sp('hl-php-func', esc(tok))
            continue
          }
          if (PHLO_FUNCS.has(lower)){
            res += sp('hl-phlo-func', esc(tok))
            continue
          }
        }
        res += esc(tok)
