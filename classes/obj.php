...
class obj implements \IteratorAggregate, \JsonSerializable {
	...
	private static array $objMC = [];
	private static array $objPC = [];
	...
	public function hasMethod($key):bool { return (self::$objMC[static::class][$key] ??= \method_exists($this, $key)) || $this->hasClosure($key); }
	public function hasProp($key):string|bool { return self::$objPC[static::class][$key] ??= (\method_exists($this, "_$key") ? "_$key" : false); }
	public function objClear():obj {
		$this->objData && [$this->objChanged = true, $this->objData = []];
		return $this;
	}
	...
	public function objImport(...$data):obj {
		foreach ($data AS $key => $value) $this->$key = $value;
		return $this;
	}
	public static function __callStatic($method, $args){
		if ($prop = (self::$objPC[static::class][$method] ??= (\method_exists(static::class, "_$method") ? "_$method" : false))) return $args ? static::$classProps[static::class][$method][\serialize($args)] ??= static::$prop(...$args) : static::$classProps[static::class][$method][void] ??= static::$prop();
	...
	public function &__get($key){
		$ref = null;
		if ($key !== 'objGet' && $this->hasMethod('objGet') && !\is_null($value = $this->objGet($key))) $ref = $value;
	...
	public function __set($key, $value){
		if ($key !== 'objSet' && $this->hasMethod('objSet') && !\is_null($this->objSet($key, $value))) return;
	...
	public function __toString(){ return $this->hasMethod('view') ? $this->view() : error('Obj "'.static::class.'" can\'t be converted to string'); }
